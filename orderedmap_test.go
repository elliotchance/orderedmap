...
func BenchmarkBigOrderedMapInitialedSize_Set(b *testing.B) {
	benchmarkBigMapInitialedSize_Set()(b)
}

func benchmarkBigMapInitialedSize_Set() func(b *testing.B) {
	m := make(map[int]bool)
	for i := 0; i < 10000000; i++ {
		m[i] = true
	}
	...
	return func(b *testing.B) {
		for j := 0; j < b.N; j++ {
			m := orderedmap.NewOrderedMap(10000000)
			for i := 0; i < 10000000; i++ {
				m.Set(i, true)
			}
		}
...
func BenchmarkAll(b *testing.B) {
	...
	b.Run("BenchmarkBigOrderedMapInitialedSize_Set", BenchmarkBigOrderedMapInitialedSize_Set)
